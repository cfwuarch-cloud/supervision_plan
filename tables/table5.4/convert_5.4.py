...
抽驗結果通知單轉換工具
======================
將 詳細價目表.xlsx 作為母本，填入 表5.4.docx 模板，
每項材料產出一份抽驗結果通知單。
...
功能說明：
  1. 讀取詳細價目表，每項材料產出一份抽驗結果通知單
  2. 填入工程名稱、材料名稱
  3. 其餘欄位保留空白（待現場填寫）

參數說明：
  -p, --price   詳細價目表 Excel 路徑（預設：../../data/02_成德-詳細價目表.xlsx）
  -t, --template  表5.4模板 docx 路徑（預設：./表5.4.docx）
  -o, --output   輸出 docx 路徑（預設：../../output/表5.4_完成.docx）
...
def main():
    parser = argparse.ArgumentParser(description='抽驗結果通知單轉換工具')
    parser.add_argument('-p', '--price', default='../../data/02_成德-詳細價目表.xlsx')
    parser.add_argument('-t', '--template', default='./表5.4.docx')
    parser.add_argument('-o', '--output', default='../../output/表5.4_完成.docx')
    ...
    for idx, (item, name, qty_unit) in enumerate(items):
        ...
        # Row 1: material name (col 1)
        if len(rows) > 1:
            fill_tc_text(rows[1], 1, name, ns)
